...
- **Disposable Design:** There is no automated backup mechanism built into this deployment. If the system enters an unresolvable or corrupted state, the primary recovery path is to completely nuke the setup using `./uninstall.sh` and build it fresh from scratch via `./install.sh`.
...
- **Privacy Constraints:** All command exchanges and configuration distributions between the admin and the bot pass directly through Telegram's commercial servers in cleartext to them. Do not utilize this setup if your threat model demands absolute transport invisibility. 